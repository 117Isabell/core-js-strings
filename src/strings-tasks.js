...
function concatenateStrings(value1, value2) {
  if (value1.length === 0 || typeof value1 !== 'string') {
    return value2;
  }
  if (value2.length === 0 || typeof value2 !== 'string') {
    return value1;
  }
...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indx).concat(str.slice(indx + value.length));
}
...
function extractNameFromTemplate(value) {
  ...
  const nameWithoutExclamation = namePart.trim().slice(0, -1);
  return nameWithoutExclamation;
}
